...
def _extract_payment_cards(html: str) -> list[dict[str, object]]:
    ...
    pans = [
        ...
        for match in re.finditer(r'<div class="list__card">.*?<span>(.*?)</span>', section, re.S)
    ]
...
def _extract_addresses(html: str) -> list[dict[str, object]]:
    ...
    addresses = [
        _strip_tags(match.group(1))
        for match in re.finditer(r'<div class="list__address"[^>]*>\s*<span>(.*?)</span>', section, re.S)
    ]
    notes = [_strip_tags(match.group(1)) for match in re.finditer(r'<span class="notes">(.*?)</span>', section, re.S)]
...
def _extract_car_plates(html: str) -> list[dict[str, object]]:
    ...
    plates = [
        _strip_tags(match.group(1)) for match in re.finditer(r'<p class="list__plate"[^>]*>(.*?)</p>', section, re.S)
    ]
