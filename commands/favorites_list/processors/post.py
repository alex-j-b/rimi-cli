"""Output shaping for favorites list."""
...
class FavoriteProductParser(HTMLParser):
...
def run(context: dict[str, object]) -> dict[str, object]:
    ...
    products_html = payload.get("productsHtml")
    if not isinstance(products_html, str):
        raise ValueError("Expected productsHtml in response")
